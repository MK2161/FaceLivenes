...
import com.example.faceliveness.MainActivity.Companion.KEY_LIVE_NESS
...
class CameraActivity : AppCompatActivity() {
        ...
        object : CountDownTimer(10*1000,1000){
            override fun onTick(millisUntilFinished: Long) {
              val seconds =((millisUntilFinished / 1000).toInt() % 60)
                formattedTime = formatSecondsInAscending(seconds)
            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        isBlink = intent.getBooleanExtra(KEY_LIVE_NESS,false)
        setUpView()
        setListeners()
    }
                ...
                outputOptions, cameraExecutor, object :ImageCapture.OnImageSavedCallback {
                    override fun onImageSaved(outputFileResults: ImageCapture.OutputFileResults) {
                        ...
                        runOnUiThread {
                            ...
                            showSnackBar("Face Successfully verified",false)
                        }
    ...
    private fun onFaceDetected(face :Face){
        Log.d(TAG,"onFaceDetected:${randomTime}")
        Log.d(TAG,"formattedTime:$formattedTime")
        Log.d(TAG,"blink :${face.leftEyeOpenProbability}: ${face.rightEyeOpenProbability}")
        Log.d(TAG,"smile :${face.smilingProbability}")
        Log.d(TAG,"checkIsRepeatedSeconds :${checkIsNonRepeatedSeconds()}")
        if (randomTime?.contains(formattedTime.toString()) == true) {
            if (checkIsNonRepeatedSeconds()){
                binding.uiTvSmile.visibility = View.VISIBLE
                completedSeconds.add(formattedTime)
                checkLiveNessProbability(face)
            } else {
                binding.uiTvSmile.visibility = View.VISIBLE
                onOverrideLastProbability(face)
            }
        } else {
            binding.uiTvSmile.visibility = View.GONE
            if (formattedTime != 0){
                if (checkIsNonRepeatedSeconds()){
                    completedSeconds.add(formattedTime)
                    checkSpoofingProbability(face)
                    if (liveNess.isNotEmpty()) onOverrideLastProbability(face)
                } else {
                    spoofNess.removeLast()
                    checkSpoofingProbability(face)
                }
            }
        }
    ...
    private fun onOverrideLastProbability(face:Face){
       val lastValue = liveNess.last()
        if (!lastValue){
            liveNess.removeLast()
            checkLiveNessProbability(face)
        }
    ...
    private fun checkLiveNessProbability(face : Face){
        when {
            if(isBlink){
                0.4 > (face.leftEyeOpenProbability ?: -10f) || (face.rightEyeOpenProbability?: -10f) < 0.4
            } else {
                0.6 < (face.smilingProbability ?: -10f)
            } -> liveNess.add(true)
            else -> liveNess.add(false)
        }
    }

    private fun checkSpoofingProbability(face: Face){
        when {
            if (isBlink){
                0.4 > (face.leftEyeOpenProbability ?: -10f ) || (face.rightEyeOpenProbability?: -10f) < 0.4
            } else {
            ...
            else -> spoofNess.add(true)
        }
    }

    private fun checkIsNonRepeatedSeconds():Boolean{
        return if (completedSeconds.isEmpty()) true
        else {
            val sec = completedSeconds.find { it == formattedTime }
            sec == null
        }
    }
    ...
    private fun checkBlinkNessProbability(){
        ...
        Log.d(TAG,"checksmileNessProbability : $liveNess")
        Log.d(TAG,"checkUnsmileNessProbability : $spoofNess")
